...
@pytest.mark.smoke
def test_admin_ping_returns_200_for_admin(admin_client, admin_token):
...
def test_admin_ping_returns_403_for_lead_and_analyst(admin_client, token):
    """QA-105-2/QA-105-3: Lead and analyst cannot access /admin/ping and get 403."""
    response = admin_client.ping(token=token)
...
def test_admin_ping_returns_401_for_missing_token(admin_client):
    """QA-105-4: Missing token cannot access /admin/ping and get 401."""
    response = admin_client.ping()
